<template>
  <div class="stimulation-timeline">
    <span
      class="targets-label"
      title="Targets of the stimulus">Targets</span>
    <edit-buttons
      class="edit-buttons"
      @itemAdd="itemAdd"
...
import mixin from 'mixins/simulation-timeline.js';
import EditButtons from 'components/run-simulation/edit-buttons.vue';
import {mapAll, unMapAll} from 'assets/utils.js';
  ...
  methods: {
    ...
    createConfig: function(config) {
      ...
      for (let i=0; i<this.items.length; i++) {
        ...
        let stimulusMapped = mapAll(this.items[i].stimulusInfo);
    ...
    loadPrevConfig: function() {
      ...
      itemsInBlueConfig.forEach((stimulusInject, index) => {
        ...
        let a = unMapAll(Object.assign({}, target, stimulus));
