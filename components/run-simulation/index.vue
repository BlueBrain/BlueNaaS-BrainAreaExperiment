        ...
        <div class="border-container">
          ...
          <div class="row">
            <stimulation-timeline
              ...
              :blue-config="blueConfig"
              class="timeline"/>
            <target-selection @targetSelected="stimulationTargetSelected"/>
          </div>
        </div>
        ...
        <div class="border-container">
          ...
          <div class="row">
            <report-timeline
              ...
              :blue-config="blueConfig"
              class="report"/>
            <target-selection @targetSelected="reportTargetSelected"/>
          </div>
        </div>
        <!-- END report timeline -->
...
import * as Unicore from 'mixins/unicore.js';
...
import {mapAll, unionArray, unMapAll} from 'assets/utils.js';
export default {
  name: 'RunSimulation',
  components: {
  ...
  methods: {
    ...
    modelTargetChanged: function(modelTarget) {
      this.modelSelected = mapAll(modelTarget);
    },
    closeTip: function() {
    ...
    checkSimulationConsistancy: function() {
      ...
      function checkMultipleEquals(arrays) {
        ...
        if (unionArray(targets).length > 1) {
          return checkModel();
        }
        return true;
      }
      ...
      try {
        if (lastConfigString) {
          ...
          if (lastConfig.blueConfig && lastConfig.runConfig) {
            ...
            this.modelSelected = unMapAll(bc.Run.Default.CircuitTarget);
